...
My data suggest that disease-associated FUS mutations impair FUS autoregulation which is normally maintained through a detained intron transcript. This work has important implications for both the RNA biology and neurodegenerative disease fields.
...
When I analyse each dataset individually, comparing knockout and NLS mutation to their respective controls in each dataset, the Dupuis dataset had the largest number of differentially expressed genes in both conditions (FDR < 0.05), an order of magnitude more than Bozzoni or Fratta. 
...
The relative weakness of NLS mutations compared to knockouts can be explained as NLS mutant FUS can still detected in the nucleus at low levels \citep{Devoy2017, Scekic-zahirovic2016}.
...
\textit{Taf15} is strongly upregulated in both MUT and KO (Fig. \ref{fig:fus_expression_multipanel}D), as is \textit{Ewsr1}.  textit{Taf15} upregulation has been repeatedly observed and validated \citep{Kino2015, Scekic-zahirovic2016}. % other citations too.
...
Xlr4b overexpression has been shown to alter dendritic spine growth in mice\citep{Cubelos2010}. 
...
These genes appear to be mouse specific.
...
Comparing the different types of splicing event shows a similar distribution between knockout-specific and overlapping splicing events, both of which dominated by complex splicing events.
...
When using tools like SGSeq that can pick up novel splicing events it can be expected that the majority of events are indeed complex,  which has been other splicing tools \citep{Vaquero-Garcia2016}.
...
Comparing the strict to the relaxed overlap criteria between the KO and MUT joint models \ref{fig:fus_splicing_multi} with the relaxed overlap criteria reduces the number of events found to be specific to FUS mutation to 16, with 405 overlapping events and 501 found to be specific to FUS knockout. 
...
Direct regulation of splicing events by FUS binding to introns has been proposed using RNA-protein interaction experiments \cite{Lagier-Tourenne2012,Rogelj2012, Ishigaki2012}.
...
This analysis shows that the role of FUS on gene expression and splicing affects mutually exclusive sets of genes.
...
The joint splicing analyses found 5 splicing events in FUS itself, all of which are classed as retained introns. 3 of the 5 are knockout-specific, hence assumed to be artefacts of partial FUS knockout in the Bozzoni dataset. 
However, The 2 remaining introns (introns 6 and 7) were found to be present in the FUS NLS mutants.
These introns overlap with large number of FUS iCLIP binding peaks.
Hence these introns may be the site of a regulation of the \textit{Fus} transcript by the binding of the FUS protein.
Many RNA-binding proteins have the ability to bind their own RNA, which creates  a negative feedback loop to tightly control the level of their own translation, a phenomenon known as autoregulation \citep{Rosenfeld2002,Jangi2014a}.
When protein levels are high, protein-RNA binding splicing towards the production of an non-coding isoform.
...
No effect on intron retention by cycloheximide treatment was seen in either wildtype of FUS $\Delta$14 homozygous fibroblasts (Fig. \ref{fig:fus_autoregulation}D).
...
These changes are less likely to be direct result of FUS RNA or protein interaction and may emerge through multiple downstream effectors or pathways.
...
As Xlr gene over-expression can alter dendritic spine growth and are regulated by the Cux1/2 transcription factors \citep{Cubelos2010}, the mechanism of how FUS loss leads to their upregulation is intriguing. 
...
Although the interaction between FUS and the U1 snRNP has been well studied \citep{Nakaya2013,Yu2015a,Yu2015b}, my analysis has uncovered a large array of splicing events which which to study the role of FUS in splicing.
...
Overall, intron retention as the true mechanism of autoregulation would make sense, due to very high evolutionary conservation of both introns and the length of FUS binding throughout.
...
My work suggests that these changes are shared due to the extreme overlap and concordance of direction of gene expression and splicing.
